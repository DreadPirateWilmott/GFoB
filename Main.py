...
def GFoB(cMenu):
	path = os.getcwd() #Get the path to the directory
	newpath = path + "\save"
	os.chdir(newpath) #Change to the save directory
	...
	while True: #Loop forever, there will be a command to end the game
		...
		if "move" in instruction or "mv" in instruction: #Ex move forward
			...
			if playerX > XZboundary or playerX < 1 or playerZ > XZboundary or playerZ < 1:
			...
			tempCompile = "%s.%s.%s.%s" % (playerX, playerY, playerZ, faceDir)
