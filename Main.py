...
#Current Version 0.0.4 - 10 hours 27 minutes of recorded time
...
def GFoB(cMenu):
	if cMenu == "start":
		...
		specialList = [] #This list contains unimportant information
		...
		searchI = "specialtiles.txt"
		if searchI in files:
			file = open("specialtiles.txt", "r")
			tempCompile = file.readline()
			...
			forZminusBound = int(tempCompile[3])
		
			specialList.append("crefor")
			file.close()
		#end if
	#end if
	while True: #Loop forever, there will be a command to end the game
		...
		if "move" in instruction or "mv" in instruction: #Ex move forward
			...
			if direction == "crefor" and "crefor" not in specialList:
				placeDis = 1
				emptyListTrig = True
			...
			#Add compiledBlock to the total tilelist
			if emptyListTrig == True:
				specialList.append("crefor")
			else:
				possibleTiles = ["stone", "dirt", "wood"] #MOD NOTICE - Any blocks created have to be added here
				if tileType not in possibleTiles or tileType not in inventory and direction != "crefor":
			...
			if compiledTile in naturalTiles:
				...
				if rsrcLck >= 1 and rsrcLck <= 10: #1 in 10 chance for coal
					inventory.append("coal")
					print "Mined 1 coal"
					oreMined = True
				elif rsrcLck > 25 and rsrcLck <= 45: #1 in 10 (about) chance for iron
			...
			import sys
			sys.exit()
	def Create(inventory): #Crafting items
		print "Create Menu"
		print "[Armour]: 17 Iron bars"
